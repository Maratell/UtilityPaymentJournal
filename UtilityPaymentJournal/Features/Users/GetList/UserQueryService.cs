...
{
    /// <summary>
    /// Реализация специализированного сервиса чтения (Read-Model) для слайса пользователей.
    /// Работает напрямую с реальным контекстом 'ApplicationDbContext', что дает ему полный доступ 
    /// к скрытым от остального приложения системным таблицам Identity (Users, Roles, UserRoles).
    /// 
    /// ЗАЧЕМ ЭТОТ КЛАСС НУЖЕН:
    /// 1. Инкапсулирует грязную инфраструктурную логику: сложные SQL/LINQ запросы, джоины и группировки 
    ///    остаются в слое Инфраструктуры, не засоряя слой бизнес-логики (Application).
    /// 2. Изолирует зависимости фреймворка: Хэндлеры MediatR теперь зависят от чистого интерфейса 'IUserQueryService',
    ///    благодаря чему их Unit-тесты не требуют настройки DbContext, MockQueryable или создания фейковых провайдеров БД.
    /// </summary>
    public class UserQueryService(ApplicationDbContext context) : IUserQueryService
    {
        public async Task<IReadOnlyCollection<GetUsersListResponse.Item>> GetUsersListWithRolesAsync(CancellationToken cancellationToken)
        {
