...
namespace UtilityPaymentJournal.Tests.Integration
{
    ...
    public class ResidencesControllerTests : BaseIntegrationTest
    {
        ...
        public async Task Create_Should_SaveResidenceInDatabase_And_ReturnCreatedStatus()
        {
            ...
            var response = await Client.PostAsJsonAsync("api/residences", request);
            ...
            var residenceInDb = await DbContext.Residences
                .IgnoreQueryFilters()
        ...
        public async Task GetById_Should_ReturnNotFound_When_ResidenceDoesNotExist()
        {
            ...
            var response = await Client.GetAsync($"api/residences/{nonExistentResidenceId}");
        ...
        public async Task GetAll_Should_ReturnResidencesList_OwnedByCurrentUser()
        {
            ...
            var myResidence = new Residence { Address = myAddress, UserId = currentUserId };
            var otherResidence = new Residence { Address = otherAddress, UserId = otherUserId };
            ...
            // ACT (Выполнение запроса)
            // ==========================================
            var response = await Client.GetAsync("api/residences");
            ...
            var content = await response.Content.ReadFromJsonAsync<GetResidencesListResponse>();
            content.Should().NotBeNull();
        ...
        public async Task Edit_Should_UpdateResidenceInDatabase_And_ReturnOkStatus()
        {
            ...
            var residence = new Residence { Address = initialAddress, UserId = testUserId };
            await DbContext.Residences.AddAsync(residence);
            await DbContext.SaveChangesAsync();
            DbContext.ChangeTracker.Clear();

            // Формируем DTO запроса на редактирование (без ID, так как ID передается в URL)
            var request = new EditResidenceRequest(updatedAddress);

            // ==========================================
            // ACT (Выполнение запроса)
            // ==========================================
            var response = await Client.PutAsJsonAsync($"api/residences/{residence.Id}", request);
            ...
            var responseContent = await response.Content.ReadFromJsonAsync<EditResidenceResponse>();
            responseContent.Should().NotBeNull();
        ...
        public async Task Delete_Should_RemoveResidenceFromDatabase_And_ReturnNoContentStatus()
        {
            ...
            var residence = new Residence { Address = addressToDelete, UserId = testUserId };
            await DbContext.Residences.AddAsync(residence);
            await DbContext.SaveChangesAsync();
            DbContext.ChangeTracker.Clear();
            ...
            // ACT (Выполнение запроса)
            // ==========================================
            var response = await Client.DeleteAsync($"api/residences/{residence.Id}");
